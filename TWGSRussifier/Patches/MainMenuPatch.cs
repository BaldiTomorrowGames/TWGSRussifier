...
namespace TWGSRussifier.Patches
{
    ...
    internal class MainMenuPatch
    {
        ...
        private static readonly List<SocialMediaInfo> SocialMediaLinks = new List<SocialMediaInfo>()
        {
            ...
            new SocialMediaInfo("GameBananaButton", "TWGS_Menu_GameBanana", "https://gamebanana.com/mods/674621"),
            new SocialMediaInfo("YouTubeButton", "TWGS_Menu_YouTube", "https://www.youtube.com/@DragTors"),
            new SocialMediaInfo("GithubButton", "TWGS_Menu_Github", "https://github.com/BaldiTomorrowGames/TWGSRussifier/tree/bbcr")
        };
        ...
        private static class MainMenuStartPatch
        {
            ...
            private static void Postfix(MainMenu __instance)
            {
                ...
                FixCursorSiblingIndex(__instance.transform);
            }
        }
        ...
        private static class SetActivePatch
        {
            ...
            {
                if (__instance.name == "Title" && value)
                {
                    ApplySizeChanges(__instance.transform);
                    ApplyLocalization(__instance.transform);
                    CreateModInfoButton(__instance.transform);
        ...
        private static void CreateModInfoButton(Transform rootTransform)
        {
            ...
            Transform? versionTransform = rootTransform.Find("Version");
            if (versionTransform == null) return;

            Transform? existingModInfo = rootTransform.Find("ModInfo");
            if (existingModInfo != null) return;

            versionTransform.gameObject.SetActive(false);

            GameObject modInfo = Object.Instantiate(versionTransform.gameObject, rootTransform);
            modInfo.name = "ModInfo";
            modInfo.SetActive(true);
            ...
            if (textComponent != null)
            {
                ...
                StandardMenuButton button = textComponent.gameObject.GetComponent<StandardMenuButton>();
                if (button == null)
                {
                    button = textComponent.gameObject.AddComponent<StandardMenuButton>();
                }
        ...
        private static void CreateSocialLinksPanel(Transform rootTransform, GameObject modInfoButton)
        {
            ...
            for (int i = 0; i < SocialMediaLinks.Count; i++)
            {
                ...
                CreateSocialButton(panel, info.ButtonName, info.LocalizationKey, info.Url, i);
            }
        ...
        private static void CreateSocialButton(GameObject parent, string buttonName, string localizationKey, string url, int index)
        {
            ...
            StandardMenuButton button = buttonObj.GetComponent<StandardMenuButton>();
            if (button == null)
            {
                button = buttonObj.AddComponent<StandardMenuButton>();
            }
        ...
        private static void ApplySizeChanges(Transform rootTransform)
        {
            ...
            {
                Transform? t = rootTransform.Find(target.Key);
                if (t != null)
                {
                    RectTransform? rt = t.GetComponent<RectTransform>();
                    if (rt != null)
                    {
                        rt.sizeDelta = target.Value;
                    }
                }
            }
        }
        ...
        private static void FixCursorSiblingIndex(Transform rootTransform)
        {
            Transform? cursor = rootTransform.Find("CursorOrigin(Clone)");
            if (cursor == null)
                cursor = rootTransform.Find("CursorOrigin");
            if (cursor != null)
                cursor.SetSiblingIndex(8);
        }
    }
}
